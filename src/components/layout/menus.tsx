    ...
    <nav className={cn(
      ...
      {menuItems.map((item) => (
        <Link
          ...
          href={item.href || "#"}
          className={cn(
            "group relative inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-muted-foreground transition-all duration-200",
            ...
            "hover:before:w-full",
            isActive(item.href || "") && "text-foreground before:w-full",
            vertical && "w-full px-3 py-2.5 before:left-0 before:top-0 before:h-full before:w-0.5 before:bottom-auto hover:before:h-full"
          )}
        >
          <span className={cn(

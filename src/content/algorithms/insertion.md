...
- **小规模数据集**：由于其简单的逻辑和低开销，对于小规模数据（如 $n < 50$），插入排序往往是最佳选择。这也是为什么高级排序算法（如 Python 的 Timsort、C++ STL 的 introsort）在递归到底层小数组时会切换为插入排序。
...
- **关注“空位”**：在代码实现中，通常不是一步步交换，而是把比 key 大的元素整个往后挪一位，腾出一个“空位”，最后把 key 放进去。这是一种优化技巧。
